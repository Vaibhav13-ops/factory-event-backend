...
describe('Factory Event Backend Tests', () => {
    ...
    test('Test 3: Different payload with older receivedTime should be ignored', async () => {
        ...
        dbManager.db.prepare('UPDATE events SET receivedTime = ? WHERE eventId = ?')
            .run('2026-01-15T12:00:00.000Z', 'E-003');
    ...
    test('Test 7: Time window boundaries should be inclusive/exclusive', async () => {
        const events = [
            {
                eventId: 'E-010',
                eventTime: '2026-01-15T10:00:00.000Z', // Exactly at start
                machineId: 'M-003',
                durationMs: 1000,
                defectCount: 1
            },
            {
                eventId: 'E-011',
                eventTime: '2026-01-15T11:00:00.000Z', // Exactly at end
                machineId: 'M-003',
                durationMs: 1000,
                defectCount: 1
            },
            {
                eventId: 'E-012',
                eventTime: '2026-01-15T10:30:00.000Z', // In the middle
                machineId: 'M-003',
                durationMs: 1000,
                defectCount: 1
            }
    ...
    test('Test 10: Health status should be calculated correctly', async () => {
        ...
        const response = await request(app)
            ...
            .query({
                machineId: 'M-HEALTH',
                start: '2026-01-15T10:00:00.000Z',
                end: '2026-01-15T11:00:00.000Z' // 1 hour window
            });
